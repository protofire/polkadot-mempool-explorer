...
class CacheService {
  ...
  static async getExtrinsics(networkId) {
    ...
    if (extrinsicKeys.length > 0) {
      const extrinsics = extrinsicKeys.map(extrinsicKey => JSON.parse(lruCache.get(extrinsicKey) || null));
  ...
  static extrinsicIsTracked(hash, from, nonce, networkId) {
    const extrinsic = CacheService.getExtrinsic(hash, from, nonce, networkId);
